...
マウスの**右ドラッグ**で画像上の領域を選択すると編集メニューが開き、選んだ編集が
その場で画像に描き込まれる。
...
続けて図形やテキストを描ける(設定は次回以降の編集にも引き継がれる):

- **線の太さ / 文字サイズ**: プリセットから選択(画面上の見た目基準。ズーム率に応じて
  画像へ換算される)
- **回転角度**: 図形・テキストを選択領域の中心周りに回転して描く(0°〜270°)
- **色の変更...**: Windows 標準の色ダイアログで自由に指定

編集は `Ctrl+Z` で1段階ずつ取り消せる(履歴は直近10回)。編集結果は `Ctrl+S` で
別ファイルに保存するか `Ctrl+C` でコピーでき、元ファイルが自動で書き換わることはない。
別の画像へ移動すると未保存の編集は破棄される。線の太さ・文字サイズ・色の初期値は
`blinker.ini` の `[edit]` セクションで変更できる。
